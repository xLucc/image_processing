...
def get_user_input():
    ...
    while True:
        cmd = input('Please insert, if the result was [b]etter, [w]orst, or [e]qual, than the last one. \n')
...
def main():
    ...
    print(f"Searchspace: guided_radius∈[1, 1280], guided_eps∈[1e-6, 1e-1], lambda∈[0.5, 3.0], clamp∈[1, 8]")
    ...
    try: 
        for i in range(n_iter):
            ...
            print(f"   lambda={sharp_lambda:.2f}, clamp={clamp * 1000:.1f}mm, radius={int(guided_radius)}, eps={eps:.2e}")

            depth_raw, color_raw = aquire_data(pipe, align)
            depth = time_filter(depth_raw)
            color = convert_guide(color_raw)
            depth_smooth = apply_filter(guide=color, target=depth, radius=guided_radius, eps=eps, unit=unit, clamp=clamp, sharp_lambda=sharp_lambda)
            ...
            history.append((int(guided_radius), eps, sharp_lambda, clamp, std_mm, rating, score))
    ...
    print()
    print('── History ───────────────────────────────────────────────────────────')
    print(f"{'#':>3}  {'radius':>6}  {'eps':>8}  {'lambda':>6}  {'clamp':>7}  {'Std':>7}  {'Rating':>9}  {'Score':>7}")
    for j, (r, eps, lam, clamp, std, rat, sc) in enumerate(history):
        rat_str = {1.0: 'better', 0.0: 'equal', -1.0: 'worse'}[rat]
        print(f'{j+1:>3}  {r:>6}  {eps:>8.2e}  {lam:>6.2f}  '
